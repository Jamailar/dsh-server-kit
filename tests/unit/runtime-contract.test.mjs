...
test('release manifest, seed locks, and Caddy trust boundary are internally consistent', async () => {
  ...
  assert.doesNotMatch(caddyfile, /header_up -X-Dsh-Proxy/)
  assert.match(caddyfile, /dsh-auth-proxy --mark-proxy/)
  assert.doesNotMatch(caddyfile, /header_up Host 127\.0\.0\.1/)
  assert.match(caddyfile, /handle \/favicon\.svg/)
  assert.match(caddyfile, /handle \/manifest\.webmanifest/)
  assert.match(caddyfile, /application\/manifest\+json/)
  assert.match(caddyfile, /versions 1\.1/)
  assert.doesNotMatch(caddyfile, /versions h1/)

  const manifestAsset = JSON.parse(await readFile(new URL('src/assets/manifest.webmanifest', projectRoot), 'utf8'))
  assert.equal(manifestAsset.name, 'DeepSeek Harness')
  assert.equal(manifestAsset.icons?.[0]?.src, '/favicon.svg')
  ...
  assert.match(entrypoint, /HOME=\$WORKSPACE_ROOT/)
  assert.match(entrypoint, /XDG_CONFIG_HOME=\$DSH_SERVER_HOME\/xdg-config/)
  assert.match(entrypoint, /XDG_DATA_HOME=\$DSH_SERVER_HOME\/xdg-data/)
